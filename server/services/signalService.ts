...
import { runTdxIndicator } from "./tdxParser";
import { sendSignalEmail } from "./emailAlert";
import { SignalRecord } from "../../drizzle/schema";
...
class SignalService {
  ...
  private async checkAllSignals() {
    ...
    try {
      ...
      const activeIndicators = indicators.filter(ind => ind.isActive && ind.pythonCode);
  ...
  private async checkIndicatorForContract(indicator: any, contract: string) {
    try {
      ...
      const klines = tqService.getMockKlines(contract, 60, 200); // 实际应从TQ获取真实数据
      
      if (!klines || klines.length === 0) return;

      // 运行指标解析
      const result = await runTdxIndicator(indicator.pythonCode, klines, indicator.name);
  ...
  private async handleTriggeredSignal(indicator: any, contract: string, signal: any, lastBar: any) {
    ...
    try {
      ...
      if (emailConfig && emailConfig.isEnabled) {
        const emailResult = await sendSignalEmail(emailConfig, record);
